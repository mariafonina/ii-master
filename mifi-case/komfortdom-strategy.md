...
2. Первичная реакция: 2,5 ч → **≤1 ч**, аварийные — по нормативу 416-ПП (5–10 мин); в цифровых каналах первый содержательный ответ — **мгновенно** (ассистент).
...
3. Прогноз повторов: бустинг на табличных признаках витрины (топ-фичи: повторы по адресу за 90 дней, просрочка SLA, доля повторов бригады, категория×сезон).

**Экономика инференса:** весь поток 14 000 обращений/мес обходится в единицы тысяч ₽/мес (расчёт в фактуре ниже) — <3% бюджета. Деньги проекта уходят на людей и интеграцию, а не на «ИИ».
...
**Бюджет 2,5 млн ₽ (ориентир):** подрядчик-разработка 1,30 / интеграция + бот MAX + веб-ассистент 0,45 / оператор заботы (5 мес) 0,225 / разметка (доплаты) 0,10 / API LLM + STT 0,075 / обучение и регламенты 0,10 / резерв 0,25.
...
Консервативный расчёт: эффект ≈ **7,7 млн ₽/год** (снижение повторных выездов ≈ 6,0 + экономия на сортировке ≈ 1,7) против 2,5 млн проекта + ≈1,15 млн/год эксплуатации → **ROI первого года ≈ 110%, окупаемость ~10 месяцев** от старта. Чувствительность: при эффекте вдвое ниже проект всё равно окупается на горизонте ~20 мес. Мягкие эффекты (NPS, удержание домов, снижение штрафных рисков по 416-ПП) в ROI не смешиваются — идут отдельным списком как апсайд.
...
- DeepSeek API (актуальная линейка и цены): {{TODO}}
- Расчёт на 14 000 обращений/мес: {{TODO}}
- Открытые модели на российском хостинге (Yandex Cloud AI Studio, Cloud.ru и др.): {{TODO}}
- Claude API: цены и доступность из РФ: {{TODO}}
- STT (SpeechKit / SaluteSpeech), стоимость расшифровки ~8 000 звонков × 4 мин: {{TODO}}
- MAX: статус, бот-платформа, референс Госуслуг: {{TODO}}
