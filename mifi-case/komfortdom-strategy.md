...
Про выбор эндпоинта — уточнение (модель одна и та же, оба варианта подключаются одинаково, по API; где стоит наш собственный сервер — роли не играет; вопрос только в том, чья инфраструктура исполняет модель и куда физически уходит текст обращения): «DeepSeek по API на российском сервере» — это ровно наш выбор, managed-эндпоинт Yandex AI Studio / Cloud.ru. Не проходит только прямой вызов api.deepseek.com, где модель исполняется в КНР — хотя Китай, вопреки интуиции, и входит в перечень «адекватных» стран РКН: нужно уведомление о трансграничной передаче, зарубежный провайдер не даст договор поручения обработки (ч. 3 ст. 6 152-ФЗ), с 01.07.2025 запрещён сбор ПДн россиян в зарубежные базы (ч. 5 ст. 18), и у DeepSeek документированные утечки (01.2025, 07.2026). Российский эндпоинт стоит ~17 тыс. ₽/мес и снимает все проблемы разом. Цифры и юрдетали — в фактуре ниже.
...
4. **152-ФЗ / трансграничка / утечка** → контур ПД в РФ, шлюз обезличивания для frontier (методы — по приказу РКН № 140, фейл-сейф на PII-детекторе), уведомление о записи звонков, минимизация ПД в витрине; аргумент дисциплины — утечка 3–15 млн ₽, повторная — оборотный штраф 1–3% выручки (с 30.05.2025).
...
- 7 жёстких вопросов руководству до старта (реконструированы 25.08 — исходный список из отчёта предпринимателя не сохранился): (1) кто владелец цифры «16% повторов» и лично отвечает за её снижение; (2) готовы ли менять регламент закрытия заявок — или нужен «просто дашборд»; (3) что сделаем с высвобожденными часами диспетчеров — сокращение, сервис, обзвоны; (4) есть ли у диспетчерской системы API/выгрузки и кто даст доступ в первую неделю; (5) кто со стороны бизнеса подпишется под guardrail «пропуск аварии ≤0,5%»; (6) принимаем ли публично правило «в пилоте никого не сокращаем»; (7) если к концу М3 эффекта нет — останавливаем проект или дотягиваем, и кто решает.
- Красные флаги (проект = выброшенные 2,5 млн): «внедрите ИИ, но процессы не трогайте» · нет владельца продукта внутри УК · «данные подрядчику давать нельзя» · ожидание, что модель всё решит сама, без регламентов.
- Статус: слой включён в презентацию — цифры вплетены в слайды 2 и 4, полный разворот — опциональный слайд 5 (приложение).
...
- Закон о развитии технологий ИИ № 243-ФЗ (подписан 26.07.2026, в силу с 01.09.2026): курс на «суверенные» модели с обработкой данных в РФ — двухконтурная архитектура кейса читается как готовность к нему на годы вперёд.
...
### Юридический бриф (152-ФЗ) — проверено ресерчером 25.08.2026

- **Трансграничная передача (ст. 12, режим с 01.03.2023).** Отдельное уведомление РКН **до начала** передачи (не путать с уведомлением об обработке по ст. 22). Перечень «адекватных» стран — приказ РКН № 128 от 05.08.2022. **Сюрприз: Китай в перечень ВХОДИТ** (раздел 2) — для передачи в КНР достаточно подать уведомление, ждать решения РКН не нужно. **США — НЕ входят:** уведомление + ожидание до 10 рабочих дней, РКН вправе запретить или ограничить передачу. Но «адекватность» Китая не делает прямой DeepSeek API легальным по умолчанию: (а) зарубежный провайдер не заключит договор поручения обработки по ч. 3 ст. 6 152-ФЗ; (б) с 01.07.2025 запрещён сбор ПДн россиян с использованием зарубежных баз (ч. 5 ст. 18, ФЗ № 23-ФЗ); (в) у DeepSeek документированные инциденты — открытая база с историями чатов (01.2025), попадание переписок в выдачу Google (07.2026), ограничение итальянским регулятором. Надзорной практики РКН именно по LLM пока нет; DeepSeek в РФ не блокировался (Ъ, 22.07.2026).
- **Обезличивание (легализация контура Б).** Корректно обезличенные данные для получателя, не имеющего «ключей», — не ПДн; режим ст. 12 на их отправку не распространяется. С 01.09.2025 процесс регулируется: ФЗ-233 + приказ РКН № 140 от 19.06.2025 — закрытый перечень методов, таблица соответствия хранится раздельно и никогда не передаётся, документированная оценка риска реидентификации, учёт операций обезличивания. Критично: вычищать и **косвенные** идентификаторы («кв. 17 в доме 5 по Ленина» идентифицирует жильца и без ФИО); обязателен фейл-сейф — блокировка отправки при срабатывании PII-детектора + регулярный аудит выборок. Остаточный риск: пропущенный шлюзом телефон = состоявшаяся трансграничная передача ПДн без уведомления. Новая публичная обязанность: по требованию Минцифры передавать обезличенные датасеты в ГИС (ЕИП НСУД, ПП № 1154 от 01.08.2025).
- **Запись звонков.** Законна при уведомлении в начале звонка (автоинформатор); продолжение разговора = согласие конклюдентными действиями. Правовое основание обработки — исполнение договора управления МКД (п. 5 ч. 1 ст. 6 152-ФЗ); цель — в политике ПДн и уведомлении РКН. Расшифровка сторонним сервисом = поручение обработки → STT только российский (SpeechKit / SaluteSpeech — подходят). Готовая формулировка: «Все разговоры записываются и обрабатываются, в том числе с использованием автоматической расшифровки, в целях приёма и исполнения ваших заявок и контроля качества обслуживания. Оператор — [наименование УК]. Продолжая разговор, вы подтверждаете ознакомление. Подробнее — в политике обработки персональных данных на сайте [адрес]».
- **Штрафы (ст. 13.11 КоАП, юрлица, с 30.05.2025):** обработка с нарушением закона — 150–300 тыс. ₽; без надлежащего согласия — 300–700 тыс.; нарушение локализации — 1–6 млн (повторно 6–18 млн); неуведомление РКН об утечке — 1–3 млн; сама утечка — 3–15 млн в зависимости от масштаба; **повторная утечка — оборотный штраф 1–3% выручки (от 20 до 500 млн ₽)**. Отдельного состава «за нарушение трансграничной передачи» в КоАП нет — квалифицируется через общие составы. Плюс уголовный контур: ст. 272.1 УК — до 8 лет за незаконный оборот ПДн с трансграничным перемещением.
- **Свежий контекст:** ФЗ № 243-ФЗ от 26.07.2026 о развитии технологий ИИ (основные положения — с 01.09.2026): категории «суверенных/национальных» моделей с обработкой данных в РФ; тотального запрета иностранных моделей для бизнеса нет, но для чувствительных сфер — переходный период до 01.09.2032. Двухконтурная архитектура кейса — запас прочности на весь горизонт.
- **Вердикт по стеку Мари:** «DeepSeek для общения» — да, но на российском хостинге: снимает разом поручение обработки, локализацию и риск утечки у зарубежного провайдера. «Claude Opus + деперсонализатор» — юридически защитимая схема при исполнении приказа № 140 (методы из перечня, раздельное хранение ключей, задокументированная оценка реидентификации, фейл-сейф), с честной оговоркой: практики РКН по таким шлюзам ещё нет, первопроходцу нужна дисциплина документирования.

---

## Открытые вопросы

1. Формат сдачи известен: ровно 4 слайда (титульный + К1 + К2 + К3), драфт — `slides-draft.md`; титул: Афонина Мария Дмитриевна, 25 августа 2026 г. Открыто: судьба слайда 5-приложения (варианты А/Б/В в ТЗ) — решение Мари.
