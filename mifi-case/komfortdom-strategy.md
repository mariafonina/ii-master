...
Почему не «просто DeepSeek по API»: Китай, вопреки интуиции, входит в перечень «адекватных» стран РКН — но прямой вызов всё равно не проходит: нужно уведомление о трансграничной передаче, зарубежный провайдер не даст договор поручения обработки (ч. 3 ст. 6 152-ФЗ), с 01.07.2025 запрещён сбор ПДн россиян в зарубежные базы (ч. 5 ст. 18), и у DeepSeek документированные утечки (01.2025, 07.2026). Та же модель на российском хостинге стоит ~17 тыс. ₽/мес и снимает все четыре проблемы разом. Цифры и юрдетали — в фактуре ниже.
